...
struct JKRAramBlock {
	...
	JKRAramBlock(uint rupaddr, uint param_2, uint param_3, byte filltype, bool mode) : blocklink(this) {
	...
	virtual ~JKRAramBlock() {
		...
		if (!pJVar1) {
			...
			void *puVar2 = pJVar1->mpData;
			*(ulong *)(puVar2 + 0x1c) = this->used + this->remainingsize + *(int *)(puVar2 + 0x1c);
			this_00->remove(&this->blocklink);
		}
		if (this != (JKRAramBlock *)0xfffffffc) {
			this->blocklink.~JSUPtrLink();
		}
	}
...
struct JKRAramHeap : public JKRDisposer {
	...
	JKRAramHeap(ulong baseaddr, ulong size) : JKRDisposer() {
		JKRHeap *pJVar1;
		JKRAramBlock *this_00;

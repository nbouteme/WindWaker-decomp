...
struct JKRFileCache : public JKRFileLoader {
	...
	struct CCacheBlock {
		...
		CCacheBlock(ulong start, ulong len, void *buffer) : linklist(this) {
			one = 1;
			start = start;
			length = len;
			buffer = buffer;
	...
	virtual bool JKRFileCache::detachResource(void *param_1) override {
		ulong uVar1;
		CCacheBlock *this_00;
		undefined4 uVar2;
	...
	virtual uint
	readResource(void *param_1, uint param_2, char *param_3) override

...
struct JUTFont {
	...
	virtual void drawChar_scale(float, float, float, float, int, bool) = 0;
	...
	virtual double drawString_size_scale(float param_1, float param_2, float param_3, float param_4, char *param_5,
											ulong param_6, bool param_7) = 0;
};
...
struct JUTResFont : public JUTFont {
	...
	void drawChar_scale(float param_1, float param_2, float param_3, float param_4, int param_5, bool param_6) {
		using namespace gx;
		ushort uVar1;
		ushort uVar2;
		ushort uVar3;
		...
		iVar7 = *(int *)((int)this->mpGlyph + (uint)this->glyphidx * 4);
	...
	void countBlock() {
		...
		do {
			...
			} else if (iVar1 < 0x4d415031) {
				/* INF1 */
				if (iVar1 != 0x494e4631) {
					if ((0x494e4630 < iVar1) || (iVar1 != 0x474c5931))
						goto LAB_802c23f8;
					this->mNumGlyph = this->mNumGlyph + 1;
				}
			} else if (iVar1 == 0x57494431) {
				this->mNumWidth = this->mNumWidth + 1;
			} else {
			LAB_802c23f8:
				JUtility::JUTReportConsole("JUTResFont: Unknown data block\n");
			}
			pRVar2 = (ResFONT *)(pRVar2->mMagic + *(int *)pRVar2->mVersion);
			blockIdx = blockIdx + 1;
		} while (true);
	}
	...
	uint getFontCode(int code) {
		...
		while (true) {
			...
			if (((int)uVar2 <= code) && (code <= (int)(uint)pMAP1->mMaxGlyph))
				break;
			idx = idx + 4;
			max = max - 1;
		}
	...
	JUTResFont(ResFONT *param_1, JKRHeap *param_2) : JUTFont() {
		initialize_state();
		...
		do {
			...
			LAB_802c314c:
				if (uVar7 != this->mNumGlyph) {
					...
					this->somevert2 = iVar6 * (uint) * (ushort *)(*(int *)((int)this->mpGlyph + iVar1) + 0xe);
	...
	void setBlock() {
		...
		do {
			...
			if (iVar3 == 0x4d415031) {
				*(ResFONT **)((int)this->mpMAP1 + iVar5) = pRVar9;
				...
				if (iVar3 == 0x494e4631) {
					...
					if ((0x494e4630 < iVar3) || (iVar3 != 0x474c5931))
						goto LAB_802c257c;
					*(ResFONT **)((int)this->mpGlyph + iVar6) = pRVar9;
					iVar6 = iVar6 + 4;
				}
			} else if (iVar3 == 0x57494431) {
			...
			// TODO: check this
			pRVar9 = (ResFONT *)(pRVar9->mMagic + *(int *)pRVar9->mVersion);
			uVar8 = uVar8 + 1;
		} while (true);
	}

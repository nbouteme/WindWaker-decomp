...
#define __OSMtGQR(gqrId, val)                   \
	asm volatile("mtspr   GQR%0,%1 # __OSMtGQR" \
				 :                              \
				 : "i"(gqrId), "b"(val))
#else
#define __OSMtGQR(gqrId, val) \
...
#define OSu8tof32(in, out) asm volatile("psq_l   %0, 0(%1), 1, 2  " \
										: "=f"(*(out))              \
										: "b"(in))
#define OSu16tof32(in, out) asm volatile("psq_l   %0, 0(%1), 1, 3  " \
										 : "=f"(*(out))              \
										 : "b"(in))
#define OSs8tof32(in, out) asm volatile("psq_l   %0, 0(%1), 1, 4  " \
										: "=f"(*(out))              \
										: "b"(in))
#define OSs16tof32(in, out) asm volatile("psq_l   %0, 0(%1), 1, 5  " \
										 : "=f"(*(out))              \
										 : "b"(in))
...
#define OSf32tou8(in, out) asm volatile("psq_st   %1, 0(%0), 1, 2 " \
										:                           \
										: "b"(out), "f"(*(in))      \
										: "memory")
#define OSf32tou16(in, out) asm volatile("psq_st   %1, 0(%0), 1, 3 " \
										 :                           \
										 : "b"(out), "f"(*(in))      \
										 : "memory")
#define OSf32tos8(in, out) asm volatile("psq_st   %1, 0(%0), 1, 4 " \
										:                           \
										: "b"(out), "f"(*(in))      \
										: "memory")
#define OSf32tos16(in, out) asm volatile("psq_st   %1, 0(%0), 1, 5 " \
										 :                           \
										 : "b"(out), "f"(*(in))      \
										 : "memory")

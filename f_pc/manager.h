...
namespace f_pc_manager {
	...
	int f_pc_manager::fpcM_IsPause(base_process_class *param_1, undefined1 param_2) {
		return f_pc_pause::fpcPause_IsEnable(param_1, param_2);
	}

	void f_pc_manager::fpcM_PauseDisable(base_process_class *param_1, byte param_2) {
		f_pc_pause::fpcPause_Disable(param_1, param_2);
		return;
	}

	int f_pc_manager::fpcM_IsCreating(uint p) {
		return f_pc_creator::fpcCt_IsCreatingByID(p);
	}

	void f_pc_manager::fpcM_Draw(void *param_1) {
		f_pc_draw::fpcDw_Execute((base_process_class *)param_1);
	}

	void fpcM_Init() {
		f_pc_layer::fpcLy_Create(&f_pc_manager::rootlayer$4264, (void *)0x0, queue$4265, 10);
		f_pc_line::fpcLn_Create();
	}

	int *f_pc_manager::fpcM_DrawIterater(f_pc_draw::DrawFunc *param_1) {
		layer_class *plVar1;
	...
	int f_pc_manager::fpcM_Delete(base_process_class *param_1) {
	...
	void *f_pc_manager::fpcM_Execute(void *param_1) {
	...
	void f_pc_manager::drawDvdCondition(long param_1) {
		undefined4 uVar1;
		JFWDisplay *pJVar2;
		ulong uVar3;
		u8 uStack246;
		...
		if (param_1 == 4) {
			f_pc_manager::messageSet(2);
		} else if (param_1 == 5) {
			f_pc_manager::messageSet(1);
		} else if (param_1 == 6) {
			f_pc_manager::messageSet(3);
		} else if (param_1 == 0xb) {
			f_pc_manager::messageSet(4);
		} else if (param_1 == 1) {
			f_pc_manager::messageSet(0);
		} else if (param_1 == -1) {
			f_pc_manager::messageSet(5);
	...
	int f_pc_manager::checkDvdCondition(void) {
		int iVar1;

		if (f_pc_manager::init$4231 == 0) {
			f_pc_manager::l_dvdError$4230 = 0;
			f_pc_manager::init$4231 = 1;
		}
		iVar1 = dvd::DVDGetDriveStatus();
		if ((iVar1) && (iVar1 != 1)) {
			f_pc_manager::l_dvdError$4230 = 1;
		}
		if (f_pc_manager::l_dvdError$4230) {
			if (iVar1 == 0) {
				f_pc_manager::l_dvdError$4230 = 0;
			} else if (*mDoRst::mResetData == 0) {
				f_pc_manager::drawDvdCondition(iVar1);
			} else {
				*mDoRst::mResetData = 0;
		...
		return f_pc_manager::l_dvdError$4230;
	}

	void f_pc_manager::fpcM_Management(code *param_1, code *param_2) {
		int iVar1;
		ulong uVar2;

		SComponent::MtxInit();
		iVar1 = f_pc_manager::checkDvdCondition();
		if (iVar1 == 0) {
			...
			f_pc_executor::fpcEx_Handler((SComponent::Method *)f_pc_manager::fpcM_Execute);
			f_pc_draw::fpcDw_Handler((f_pc_draw::DrawFunc *)f_pc_manager::fpcM_DrawIterater, (f_pc_draw::DrawArg *)f_pc_manager::fpcM_Draw);
			if (param_2) {
				(*param_2)();
			}
		}

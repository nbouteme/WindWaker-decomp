...
JKRExpHeap::CMemBlock *JKRExpHeap::CMemBlock::allocBack(ulong param_1, uchar param_2, uchar param_3, uchar param_4, uchar param_5) {
	...
	iVar1 = nullptr;
	if (this->addrsize < param_1 + sizeof(CMemBlock)) {
		this->fill3 = param_4;
		this->fill2 = 0x80;
	} else {
		...
		this->fill3 = param_2;
		this->fill2 = param_3;
		this->addrsize = this->addrsize - (param_1 + sizeof(CMemBlock));
	}
	return iVar1;
}
...
void JKRExpHeap::joinTwoBlocks(CMemBlock *param_1) {
	...
	if (uVar4 == uVar2) {
		param_1->addrsize = pCVar3->addrsize + ((byte)pCVar3->fill2 & 0x7f) + param_1->addrsize + sizeof(CMemBlock);
...
int JKRExpHeap::getTotalUsedSize() {
	...
	for (pCVar1 = this->head; pCVar1; pCVar1 = pCVar1->next) {
		iVar2 += pCVar1->addrsize + sizeof(CMemBlock);
	}
...
JKRHeap *JKRExpHeap::createRoot(int param_1, bool param_2) {
	...
	if (JKRHeap::sRootHeap == (JKRHeap *)0x0) {
		...
		pJVar1 = local_14[0];
		if (local_14[0]) {
			new (local_14[0]) JKRExpHeap(local_14[0] + 1, local_18 - sizeof(JKRExpHeap), (JKRHeap *)0x0, param_1);
			pJVar2 = local_14[0];
			pJVar1 = local_14[0];
		}
	}
...
JKRExpHeap *JKRExpHeap::create(ulong param_1, JKRHeap *param_2, bool param_3) {
	...
	if (size < 0xa0) {
		...
		if (self) {
			pJVar1 = self;
			if (self) {
				new (self) JKRExpHeap(self + 1, size - sizeof(JKRExpHeap), param_2, param_3);
			}
			if (pJVar1 == nullptr) {
...
JKRExpHeap::JKRExpHeap(void *param_1, ulong param_2, JKRHeap *param_3, bool param_4) : JKRHeap(param_1, param_2, param_3, param_4) {
	...
	//this->freelist->initiate(nullptr, nullptr, param_2 - sizeof(CMemBlock), 0, 0);
	this->head = nullptr;
	this->tail = nullptr;
}
...
JKRExpHeap::CMemBlock *JKRExpHeap::allocFromTail(uint param_1, int param_2) {
	...
	do {
		...
		LAB_802b1cdc:
			if (pCVar5 == nullptr) {
				pCVar4 = nullptr;
			} else if (uVar6 < sizeof(CMemBlock)) {
				if (uVar6 == 0) {
				...
				pCVar4->initiate(nullptr, nullptr, unaff_r28, this->gid, 0x80);
				pCVar5->addrsize = (pCVar5->addrsize - unaff_r28) - sizeof(CMemBlock);
				appendUsedList(pCVar4);
				pCVar4 = pCVar4 + 1;
			}
			return pCVar4;
...
JKRExpHeap::CMemBlock *JKRExpHeap::allocFromHead(uint param_1, int param_2) {
	...
	if (pCVar9 == nullptr) {
		...
		pCVar7 = pCVar9->prev;
		pCVar5 = pCVar9->next;
		pCVar2 = (CMemBlock *)pCVar9->allocFore(uVar6 - sizeof(CMemBlock), '\0', '\0', '\0', '\0');
		if (pCVar2 == nullptr) {
			pCVar3 = nullptr;
		} else {
...
void JKRExpHeap::do_freeAll() {
	...
	this->freelistend = this->freelist;
	this->freelist->initiate(nullptr, nullptr, this->mSize - sizeof(CMemBlock), 0, 0);
	this->tail = nullptr;
	...
	if (((this_00 == nullptr) || (param_1 < mpDataBegin)) ||
		...
		if (uVar1 == uVar3) {
			...
			if (uVar3 < uVar1) {
				...
				this_00->addrsize = pCVar4->addrsize + this_00->addrsize + sizeof(CMemBlock);
				if ((sizeof(CMemBlock) < this_00->addrsize - uVar1) &&
					(pCVar2 = (CMemBlock *)
								  this_00->allocFore(uVar1, this_00->fill3, this_00->fill2, '\0', '\0'),
					 pCVar2)) {
					recycleFreeBlock(pCVar2);
				}
			} else if ((sizeof(CMemBlock) < uVar3 - uVar1) &&
					   (pCVar2 = (CMemBlock *)
									 this_00->allocFore(uVar1, this_00->fill3, this_00->fill2, '\0', '\0'),
						pCVar2)) {
				recycleFreeBlock(pCVar2);
	...
	do {
		...
		LAB_802b2a30:
			for (pCVar6 = this->freelist; pCVar6 != (CMemBlock *)0x0; pCVar6 = pCVar6->next) {
				iVar8 = pCVar6->addrsize + iVar8 + sizeof(CMemBlock);
				if (pCVar6->next == (CMemBlock *)0x0) {
					...
					if (pCVar6->next < (CMemBlock *)((u64)&pCVar6[1].id + pCVar6->addrsize)) {
						iVar7 = 0;
		...
		puVar2 = &pCVar6->fill2;
		pCVar6 = pCVar6->next;
		iVar8 = *puVar1 + ((byte)*puVar2 & 0x7f) + iVar8 + sizeof(CMemBlock);
	} while (true);
}

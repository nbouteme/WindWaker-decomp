...
namespace gx {
	...
	struct {
		...
		UniformDesc init() {
			...
			int n;
			glGetProgramInterfaceiv(ubershader, GL_SHADER_STORAGE_BLOCK, GL_ACTIVE_RESOURCES, &n);

			uint a = glGetProgramResourceIndex(ubershader, GL_SHADER_STORAGE_BLOCK, "_vertices");
			printf("a: %d, n: %d\n", a, n);
	...
	*/
	...
	void GXFlush() {
		if (glState.pendingDraw) {
			glState.updateGX();
			glFlushMappedNamedBufferRange(glState.vbo, 0, 4 * 1024 * 1024);
			glFlushMappedBufferRange(GL_SHADER_STORAGE_BUFFER, 0, 4 * 1024 * 1024);
			glFlushMappedNamedBufferRange(glState.gxubo, 0, glState.gxubosize);
			glFlushMappedBufferRange(GL_UNIFORM_BUFFER, 0, glState.gxubosize);
...
#define GXCDEC(prfx, n, t) GXCDEC##n(prfx##n##t, t, t)
	...
	template <typename T, T vertex_attr::*ptr, bool inc = false>
	struct GXAttrWriter {
		void write(T data) {
			// todo: handle indirect modes
			if constexpr (inc) {
				glState.currentVertSpec++;
				...
				if (glState.currentPType == GXPrimitive::GX_QUADS) {
					if (glState.currentSpecifiedPosCnt &&
						...
						glState.currentVertSpec[0] = glState.currentVertSpec[-1];
					}
				}
			}
	...
	GXCDEC(GXPosition, 3, f32) {
		...
		glState.currentVertSpec[0].col[0][0] = 0;
		glState.currentVertSpec[0].col[0][1] = 1;
		glState.currentVertSpec[0].col[0][2] = 1;
		glState.currentVertSpec[0].col[0][3] = 1;

		glState.currentVertSpec[1].pos[0] = 640;
		glState.currentVertSpec[1].pos[1] = 0;
		glState.currentVertSpec[1].pos[2] = 0;
		glState.currentVertSpec[1].col[0][0] = 1;
		glState.currentVertSpec[1].col[0][1] = 1;
		glState.currentVertSpec[1].col[0][2] = 0;
		glState.currentVertSpec[1].col[0][3] = 1;

		glState.currentVertSpec[2].pos[0] = 640;
		glState.currentVertSpec[2].pos[1] = 480;
		glState.currentVertSpec[2].pos[2] = 0;
		glState.currentVertSpec[2].col[0][0] = 1;
		glState.currentVertSpec[2].col[0][1] = 0;
		glState.currentVertSpec[2].col[0][2] = 1;
		glState.currentVertSpec[2].col[0][3] = 1;
	}

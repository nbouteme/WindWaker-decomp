...
namespace m_Do_graphic {
...
#else
		// reject fragments that don't pass the alpha test
		// disable zbuffer
		// default blend
		// default alpha compar
		//// local_8[0] = d_com_inf_game::g_clearColor;
		// disable fog
	...
	void drawAlphaBuffer(camera_class *param_1, gx::GXColor *param_2) {
		...
		gx::GXPosition3s8(0, 0, -5);
		gx::GXPosition3s8(1, 0, -5);
		gx::GXPosition3s8(1, 1, -5);
		gx::GXPosition3s8(0, 1, -5);
...
// disable z compare
// destalpha
// alpha compare = always
// disable fog, culling
// no ind stage
// ortho matrix in [0,1], [0, 1], [0, 10] space

// draw a quad spanning the screen at half depth?
// Vertex format is [Pos(3b)]
...
// restore camera projection
	...
	void clearAlphaBuffer(camera_class *param_1, byte param_2) {
		...
		gx::GXColor local_50 = {0, 0, 0, param_2};
		drawAlphaBuffer(param_1, &local_50);
#endif
		return;
	}
